...
function MobilePayement(props: Props) {
    ...
    const submitRetrait = async (retraitId:string) => {
          message.loading("Transfert de fond en cours")
          .then(async () => {
  ...
  const delRechargement = async (depositId:string) => {
    ...
    const columns = [
        { title: 'N° Commande', dataIndex: '_id', key:'_id',fixed: true, render: (info:any) => {
            ...
            return (
                <Buttons
                                        ...
                                        onClick={()=> {
                                            message.loading("Activation en cours")
                                            .then(async () => {
      ...
      const columnsScecond = [
        { title: 'Reseau', dataIndex: 'typeCrypto', key:'typeCrypto',fixed: true },
        { title: 'Reference', dataIndex: 'ref', key:'ref' ,fixed: true },
        { title: 'Montant Net', dataIndex: 'montant_send_net_currencie', key:'montant_send_net_currencie',fixed: true  },
        { title: 'Numero', dataIndex: 'numero', key:'numero' },
        
        
        { title: 'Etat', dataIndex: 'state', key:'state',render: (info:any) => {
          const state = info as number;
          if(state === 1) {
            return (
              <Tag color={'volcano'}>
                Client dans l'attente
              </Tag>
            );
          } else {
            return (
      ...
      const columnsRetrait = [
        ...
        { title: 'Montant Local', dataIndex: 'montant_give_of_client_local_currencie', key:'montant_give_of_client_local_currencie',fixed: true  },
        { title: 'Montant USD', dataIndex: 'montant_give_of_client_usd', key:'montant_give_of_client_usd' },
        { title: 'Type', dataIndex: 'typeCrypto', key:'typeCrypto' },
        { title: 'Adresse', dataIndex: 'address', key:'address' },
        { title: 'Date Operation', dataIndex: 'registerDate', key:'registerDate', render: (element:string) => <p key={element}>{format(new Date(element), "dd/MMM/yyyy HH:mm")}</p> },
        ...
        { title: 'Action', key:'_id',render: (element:string,rowData:any) => {
          return  <Buttons
                    ...
                    tooltip="Valider Retrait"
                    onClick={async() => {await submitRetrait(rowData._id);}}
                />;
              }
        },
      ];
                                ...
                                <Tabs>
                                    <TabPane tab="Manage Rechargement" key="1">
                                          ...
                                          <div className={classes.root}>
                                              <Accordion defaultExpanded>
                                                  <AccordionSummary
                                                      ...
                                                      <div className={classes.column}>
                                                          <Typography className={classes.secondaryHeading}>Enregistrer une transaction Rechargement</Typography>
                                                      </div>
                                                  </AccordionSummary>
                                                      ...
                                                      <Grid container spacing={1} style={{padding: 10}}>
                                                          <Grid item xs={3}>
                                                              <TextInputField
                                                                              ...
                                                                              onChange={(e) => changeNumeroRechargement(e.target.value)}
                                                                          />
                                                          ...
                                                          <Grid item xs={3}>
                                                            <Select
                                                              ...
                                                              showSearch
                                                              >
                                                              {reseau.map(value => (<Option key={value} value={value}>{value.toLocaleUpperCase()}</Option>))}
                                                            </Select>
                                                          </Grid>
                                                      </Grid>
                                      ...
                                      <Grid container spacing={1} style={{padding: 20}}>
                                          <div className={classes.root}>
                                          <div className={classes.column}>
                                              <Typography className={classes.secondaryHeading}>List Rechargement en attente</Typography>
                                              <Table columns={columnsScecond} dataSource={rechargement} />
                                          </div>
                                          </div>
                                      </Grid>
                                    </TabPane>
                                    ...
                                    <TabPane tab="Manage Numero" key="3">
                                          ...
                                          <div className={classes.root}>
                                              <Accordion defaultExpanded>
                                                  <AccordionSummary
                                                      ...
                                                      <div className={classes.column}>
                                                          <Typography className={classes.secondaryHeading}>Ajouter un Mobile Money</Typography>
                                                      </div>
                                                  </AccordionSummary>
                                                  <AccordionDetails className={classes.details}>
                                                      <Grid container spacing={1} style={{padding: 10}}>
                                      ...
                                      <Grid container spacing={1} style={{padding: 20}}>
                                          <div className={classes.root}>
                                          <div className={classes.column}>
                                              <Typography className={classes.secondaryHeading}>List Mobile Money</Typography>
