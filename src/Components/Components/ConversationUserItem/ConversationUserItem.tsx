...
import {ListItem, Divider, ListItemText, ListItemAvatar, ListItemButton, Avatar, Typography} from '@mui/material';
...
export default function ConversationUserItem(props: Props) {
    const lastWrite = props.room.split('_')[0] === props.lastContent.ident ? "Vendeur": "Acheteur";
    const content = props.lastContent.content !== "" ? 
        props.lastContent.content.length <= 48 ? 
            props.lastContent.content : `${props.lastContent.content.substring(0,47)}...`
        : props.lastContent.image.indexOf('.m4a') !== -1 ?
            '🎤 une note vocale a été envoyé' : '🖼️ une image a été envoyé';
        let tag:any;
        switch (props.etatCommunication) {
            case EtatCommunication.BEGIN:
                tag = <Tag icon={<SyncOutlined spin />} color="processing">En négociation</Tag>;
                break;

           case EtatCommunication.ACCORD_SELLER:
                tag = <Tag icon={<ExclamationCircleOutlined />} color="warning">Proposition vendeur</Tag>;
                break;
            case EtatCommunication.ACCORD_SELLER_BUYER:
                tag = <Tag icon={<CheckCircleOutlined />} color="success">Accord vendeur/acheteur</Tag>;
            break;
            case EtatCommunication.SELLER_UNAVAILABLE:
                tag = <Tag icon={<MinusCircleOutlined />} color="default">Produit non recupéré</Tag>;
                break;
            case EtatCommunication.BUYER_NOT_VALIDATE:
                tag = <Tag icon={<CloseCircleOutlined />} color="error">Client insatisfait</Tag>;
                break;
            default:
