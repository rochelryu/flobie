  ...
  const validateTravelDemande = () => {
    if (assuranceExpiration.length > 6 && techniqueExpiration.length > 6) {
      message.loading("Validation en cours").then(async () => {
        const data = await consumeApi.validateTravelDemande(
          itemSelect,
          assuranceExpiration,
          techniqueExpiration
        );
        if (data.etat === Etat.SUCCESS) {
          ...
          const resultTravellersDemandes = data.result
            .allTravellersDemandes as any[];
          const allTravellersDemandes = resultTravellersDemandes.map(
            (value) => {
              return { ...value, key: value._id };
            }
          );
      ...
      message.error(
        "Les dates d'expirations ne sont pas renseignées correctement"
      );
    }
  };
          ...
          <div className="pt-10">
            ...
            <Grid container spacing={1} style={{ padding: 20 }}>
              <div className={classes.root}>
                <div className={classes.column}>
                  <Typography className={classes.secondaryHeading}>
                    List Demande Conducteur en cours
                  </Typography>
                  <Table
                    columns={columns}
