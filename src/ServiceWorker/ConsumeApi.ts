...
export class ConsumeApi {
    ...
    private manageMobileMoney_url = this.base_url + "/lastLevel/manageMobileMoney";
    private getDetailsCommandes_url = this.base_url + "/lastLevel/getDetailsCommandes";
    private getDetailsTravellerDemande_url = this.base_url + "/lastLevel/getDetailsTravellerDemande";
    private getDetailsPassagersDemande_url = this.base_url + "/lastLevel/getDetailsPassagersDemande";
    private setActuality_url = this.base_url + "/lastLevel/createActuality";
    private setDeleveryProductLevelOne_url = this.base_url + "/lastLevel/setDeleveryProductLevelOne";
    ...
    async changePassword(old_password: string,new_password: string): Promise<ResponseInterface> {
    ...
    async createAdmin(name: string,email: string, prefix: string, role: string, numero: string, password: string): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async createCategorie(name: string,domaine: number[], popularity:number): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async togglePopularityCategorie(categoryId: string): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async retraitReserve(typeDeReserve: number, amountUsd:number): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async validateTravelDemande(itemSelect: string,assuranceExpiration: string,techniqueExpiration: string  ): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async validatePassagerDemande(itemSelect: string ): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async rembourseClient(itemSelect: string, typeOfRetour:number): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async setDeleveryProductLevelTwo(itemSelect: string): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        const body = {
            id,
    ...
    async createActuality(autherName: string,authorProfil: string, categorie: string, imageCover: string, title: string, sights: any[]): Promise<ResponseInterface> {
        const id = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
    ...
    async getEmployer(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.employer_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getActualities(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getActualities_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getCommandes(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getCommandes_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getTravellersDemandes(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getTravellersDemandes_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getPassagersDemandes(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getPassagersDemandes_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getManagementMobileMoney(): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.manageMobileMoney_url}?ident=${ident}&recovery=${recovery}`);
    }

    async getDetailsCommandes(idReservation: string): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getDetailsCommandes_url}?ident=${ident}&recovery=${recovery}&idReservation=${idReservation}`);
    }

    async getDetailsTravellerDemande(idClient: string): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getDetailsTravellerDemande_url}?ident=${ident}&recovery=${recovery}&idClient=${idClient}`);
    }

    async getDetailsPassagersDemande(idClient: string): Promise<ResponseInterface> {
        const ident = localStorage.getItem('ident');
        const recovery = localStorage.getItem('recovery');
        return await get(`${this.getDetailsPassagersDemande_url}?ident=${ident}&recovery=${recovery}&idClient=${idClient}`);
    }
}